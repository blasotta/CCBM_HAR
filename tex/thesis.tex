...
where some arbitrary but fixed ordering of the variables is assumed (correct?). This property can be exploited in order to estimate the joint distribution of variables. To perform this task, many methods exist. In the line of this work we will focus on methods based feed-forward neural networks which fulfill certain properties that allow correct estimation of probability densities.

\section{Symbolic Reasoning}
\label{sec:evil}
(TODO: Preliminaries of / Introduction to Computational State Space models here)
The goal of human activity recognition is to recognize certain activities in a sequence of actions that a human could perform. As such the HAR context is always a dynamic one, i.e. entities within this system are subject to changing states starting from some initial state. Due to combinatorial explosion the state space is potentially very large. Thus, a system which allows tractable inference in dynamic systems is required. In a feasibility study, Computational State Space models (CSSMs) have been shown to fulfill this property, even in complex, real-world applications \cite{kruger_computational_2014}. The Authors found that the domain model used in their evaluation had circa $10^8$ states.
